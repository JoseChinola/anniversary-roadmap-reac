...
const ParticlesBackground = () => {
  ...
  useEffect(() => {
    const createParticle = () => {
      ...
      setParticles((prev) => [
        ...prev,
        { id: Date.now(), size, left, duration, color },
      ]);
...
const PhotoCollage = () => {
  ...
  return (
    <section
      id="gallery"
      className="relative py-24 bg-gradient-to-b from-white via-pink-50 to-pink-100 overflow-hidden"
    >
      <Confetti />
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-8">
        {photos.map((src, idx) => (
          ...
          >
            ...
            />
            <div className="absolute bottom-2 left-0 right-0 text-center text-xl text-gray-400 italic">
              ♥ Recuerdo {idx + 1}
            </div>
          </motion.div>
        ))}
